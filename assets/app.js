(() => {
  ...
  function buildDaySteps(day) {
    if (isTimedBlockDay(day)) {
      const minutes = day.amrapMinutes || day.emomMinutes;
      return [{
        type: "timedBlock",
        mode: day.amrapMinutes ? "amrap" : "emom",
        seconds: minutes * 60,
        checkKey: "block",
      }];
    }
  ...
  function renderDay(dayNum) {
    ...
    if (isTimedBlockDay(day)) {
      list.appendChild(timedBlockRow(day));
    } else {
      let lastRound = -1;
  ...
  function timedBlockRow(day) {
    const st = dayState(day.day);
    const tKey = `${day.day}-block`;
    const seconds = (day.amrapMinutes || day.emomMinutes) * 60;
    const isAmrap = !!day.amrapMinutes;
    ensureTimer(tKey, seconds, day.title);

    const card = document.createElement("div");
    card.className = "timed-block";

    const head = document.createElement("div");
    head.className = "timed-block__head";
    head.innerHTML = `
      <span class="timed-block__badge">${isAmrap ? "AMRAP" : "EMOM"}</span>
      <span class="timed-block__duration">${formatDuration(seconds)} totales</span>
    `;
    card.appendChild(head);

    const t = timers[tKey];
    const timerWrap = document.createElement("div");
    timerWrap.className = "timed-block__timer" + (t.running ? " is-running" : "");

    const display = document.createElement("div");
    display.className = "timed-block__display";
    display.textContent = formatClock(Math.max(t.remaining, 0));

    const bar = document.createElement("div");
    bar.className = "timed-block__bar";
    const fill = document.createElement("div");
    fill.className = "timed-block__bar-fill";
    bar.appendChild(fill);

    const controls = document.createElement("div");
    controls.className = "timed-block__controls";
    const playBtn = document.createElement("button");
    playBtn.className = "timer__btn timer__btn--lg";
    playBtn.setAttribute("aria-label", "Iniciar o pausar");
    playBtn.textContent = t.running ? "⏸" : "▶";
    const resetBtn = document.createElement("button");
    resetBtn.className = "timer__btn";
    ...
    registerDisplay(tKey, (tt) => {
      ...
      timerWrap.classList.toggle("is-running", tt.running);
    });

    playBtn.addEventListener("click", () => {
      if (t.running) pauseTimer(tKey); else { startTimer(tKey); showFloating(tKey); }
    });
    resetBtn.addEventListener("click", () => resetTimer(tKey));
    ...
    refList.className = "timed-block__list";
    refList.innerHTML =
      `<div class="timed-block__list-label">${isAmrap ? "Ronda a repetir sin parar hasta que suene:" : "Ejercicios a hacer cada minuto en punto:"}</div>` +
      day.exercises.map(ex => `
        <div class="timed-block__item">
          <span>${ex.name}${ex.scale ? `<div class="exercise__scale">${ex.scale}</div>` : ""}</span>
          <span class="timed-block__item-amount">${ex.reps ? `${ex.reps} reps` : formatDuration(ex.seconds || 0)}</span>
        </div>
      `).join("");
    card.appendChild(refList);

    const checked = !!st.checks["block"];
    const doneBtn = document.createElement("button");
    doneBtn.className = "complete-box" + (checked ? " is-done" : "");
    doneBtn.style.marginTop = "16px";
    doneBtn.innerHTML = `
      <span class="complete-box__check">✓</span>
      <span>
        <div class="complete-box__label">${checked ? "Bloque marcado como hecho" : "Marcar bloque como hecho"}</div>
        <div class="complete-box__sub">Se marca solo al acabar el temporizador, o puedes marcarlo tú.</div>
      </span>
    `;
    doneBtn.addEventListener("click", () => {
      st.checks["block"] = !st.checks["block"];
  ...
  function renderWorkoutStep() {
    ...
    if (step.type === "exercise") {
      ...
      startTimer(tKey);
    } else if (step.type === "timedBlock") {
      const meta = TYPE_META[day.type];
      wrap.style.setProperty("--type-color", meta.color);
      wrap.innerHTML = `
        <div class="workout-step__badge">${step.mode === "amrap" ? "AMRAP" : "EMOM"}</div>
        <h2 class="workout-step__name">${day.title}</h2>
        <div class="workout-step__scale">${step.mode === "amrap" ? "Repite la ronda de abajo sin parar hasta que suene." : "Haz la ronda de abajo cada minuto en punto."}</div>
      `;
      const tKey = `${day.day}-${step.checkKey}`;
      ensureTimer(tKey, step.seconds, day.title);
      workoutActiveTKey = tKey;
      wrap.appendChild(bigTimerDisplay(tKey));
      const refList = document.createElement("div");
      refList.className = "workout-step__ref-list";
      refList.innerHTML = day.exercises.map(ex => `
        <div class="workout-step__ref-item">
          <span>${ex.name}</span>
          <span>${ex.reps ? `${ex.reps} reps` : formatDuration(ex.seconds || 0)}</span>
        </div>
      `).join("");
      wrap.appendChild(refList);
      wrap.appendChild(workoutSkipBtn("Terminar antes ▶"));
      startTimer(tKey);
    }
  ...
  function tick(tKey) {
    ...
    if (t.remaining <= 0) {
      ...
      pauseTimer(tKey);
      beep();
      if (tKey.endsWith("-block")) {
        const dayNum = parseInt(tKey.split("-")[0], 10);
        const st = dayState(dayNum);
        st.checks["block"] = true;
        st.doneOverride = null;
        saveProgress();
        if (currentDay === dayNum && !workoutState) renderDay(dayNum);
      }
